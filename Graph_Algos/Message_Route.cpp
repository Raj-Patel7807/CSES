...
void DFS(ll node, ll final, vector<vector<ll>> &graph, vector<ll> &visited, ll& ans, ll cnt, vc<ll>& route, vc<ll> tmp) {
    visited[node] = 1; tmp.PUSH(node);
    if(node == final) {
        if(ans > cnt) {
            ans = cnt;
            route = tmp;
        ...
        for(ll neighbor : graph[node]) if(!visited[neighbor]) {
            DFS(neighbor, final, graph, visited, ans, cnt+1, route, tmp);
        }
    }
    visited[node] = 0; tmp.POP();
}
...
void Raj_Patel_7807(ll tt) {
    ...
    vc<ll> vis(n+1, 0), par(n+1, -1), dist(n+1, INF), ans;
    queue<ll> q; dist[1] = 1;
    q.push(1);
    while(!q.empty()) {
        ll x = q.front(); q.pop();
        FORE(i, gr[x]) if(dist[i] == INF) {
            dist[i] = dist[x] + 1;
            par[i] = x; q.push(i);
        }
    }
    if(dist[n] == INF) { OUT("IMPOSSIBLE"); return; }
    ll node = n;
    while(node != -1) {
        ans.PUSH(node);
        node = par[node];
