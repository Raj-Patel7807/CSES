...
template <class... T> inline void INPUT(T&... args) { ((cin >> args), ...); }
template <class... T> inline void OUTPUT(bool F, T&&... args) { ((cout << args << (F ? '\n' : ' ')), ...); }
...
template <class T> inline ll minid(const vector<T>& v) { return (min_element(all(v)) - v.begin()); }
template <class T> inline ll maxid(const vector<T>& v) { return (max_element(all(v)) - v.begin()); }
...
bool BFS(ll root, vector<vector<ll>> &graph, vector<ll> &visited, vc<ll>& color) {
    queue<ll> q; q.push(root); visited[root] = 1;
    ll tmp = 0; color[root] = tmp;
    while(!q.empty()) {
        ll node = q.front(); q.pop();
        for(ll neighbor : graph[node]) {
            if(!visited[neighbor]) {
                if(color[neighbor] == -1) color[neighbor] = !color[node];
                else return false;
                visited[neighbor] = 1; q.push(neighbor);
            }
        }
    } return true;
}
void DFS(ll node, ll col, vector<vector<ll>> &graph, vector<ll> &visited, vc<ll>& color, bool& ok) {
    visited[node] = 1; color[node] = col;
    for(ll neighbor : graph[node]) {
        if(!visited[neighbor]) { DFS(neighbor, 3-col, graph, visited, color, ok); }
        else {
            if(color[neighbor] == color[node]) ok = false;
        }
    }
}
...
void Raj_Patel_7807(ll tt) {
    ...
    FOR(m) {
        IN(ll, x, y);
        gr[x].PUSH(y); gr[y].PUSH(x);
    } debug(gr);
    vc<ll> vis(n+1, 0), color(n+1, -1);
    FOR(n) {
        if(!vis[i+1]) {
            bool ok = true;
            DFS(i+1, 1, gr, vis, color, ok);
            if(!ok) {
                OUT("IMPOSSIBLE"); return;
            }
        }
    }
    FOR(n) { OUTT(color[i+1]); } ln;
}
