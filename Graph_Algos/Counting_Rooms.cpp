...
void DFS(ll i, ll j, vector<vector<char>> &graph, vector<vc<bool>> &visited) {
    visited[i][j] = true;
    ll n = LEN(graph), m = LEN(graph[0]);
    vc<pr<ll, ll>> dir = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};
    FORE(x, dir) {
        if(i+x.FF < n && i+x.FF >= 0 && j+x.SS < m && j+x.SS >= 0 && graph[i+x.FF][j+x.SS] == '.' && !visited[i+x.FF][j+x.SS]) {
            DFS(i+x.FF, j+x.SS, graph, visited);
        }
    }
}
...
void Raj_Patel_7807(ll tt) {
    IN(ll, n, m); VVIN(char, a, n, m);
    ll ans = 0;
    vc<vc<bool>> vis(n, vc<bool>(m, false));
    FOR(n) FOR(j, m) if(a[i][j] == '.' && !vis[i][j]) {
        DFS(i, j, a, vis); ans++;
    } OUT(ans);
}
