...
void solve(ll tt) {
    ...
    FOR(tot) {
        if(!i) { OUT(string(n, '0')); continue; }
        ll m = i; string bin = "", gry = "";
        while(m) {
            bin += (char)((m & 1) + '0');
            m >>= 1;
        } reverse(all(bin)); debug(bin);
        if(LEN(bin) < n) bin = string(n-LEN(bin), '0') + bin; debug(bin);
        gry += bin[0];
        FOR(j, 1, LEN(bin)) gry += (char)(((bin[j]-'0') ^ (bin[j-1]-'0')) + '0');
        OUT(gry); debug(gry);
    }
}
