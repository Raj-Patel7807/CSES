...
using ppll = pair<pair<ll, ll>, ll>;
using pllp = pair<ll, pair<ll, ll>>;
using mll = map<ll, ll>;
...
vector<ll> getPrimeDivisors(ll n) {
    vector<ll> primeDivisors; for(ll i=2; i*i<=n; i++) { while(n % i == 0) { primeDivisors.push_back(i); n /= i; } }
    if(n > 1) { primeDivisors.push_back(n); } return primeDivisors;
}
void precomputeDivisors(ll n) { for(ll i=1; i<=n; i++) { for(ll j=i; j<=n; j+=i) { Divisors[j].push_back(i); } } }
...
bool comp(const pr<ll, ll>& a, const pr<ll, ll>& b) { return a.ss < b.ss; }

//==========^==========<<   C O D E   B Y   R A J  P A T E L   >>==========^==========//

void solve(ll tt) {
    in(ll, n);
    if(n*(n+1)/2 % 2) { out(0); return; }
    vvll dp(n+1, vll(n*(n+1)/2+2, -1));

    function<ll(ll, ll)> rec = [&](ll id, ll sum) ->ll {
        if(!sum) return 1;
        if(id > n || sum < 0) return 0;
        if(dp[id][sum] != -1) return dp[id][sum];
        ll notTake = rec(id+1, sum);
        ll take = rec(id+1, sum-id);
        return dp[id][sum] = (take + notTake) % MOD;
    };
    out((rec(1, n*(n+1)/4) * modInv(2)) % MOD);
}
