...
#define inn(T, ...) T __VA_ARGS__; read(__VA_ARGS__);
#define outt(...) write(true, __VA_ARGS__);
#define outtt(...) write(false, __VA_ARGS__);
...
private:
    struct Node {
        ll sum, lazyAdd, lazySet, mn;
        Node() : sum(0), lazyAdd(0), mn(INF), lazySet(LLONG_MIN) {}
    };
...
public:
    ...
    void build(ll idx, ll low, ll high, const vector<ll>& arr) {
        if(low == high) { seg[idx].mn = arr[low]; return; }
        ll mid = (low + high)/2;
...
void solve(ll tt) {
    ...
    fo(q) {
        ...
        if(ty == 1) {
            inn(ll, id, v);
            st.updateSet(0, 0, n-1, id-1, id-1, v);
        } else {
            inn(ll, l, r);
            outt(st.query(0, 0, n-1, l-1, r-1).mn);
        }
    }
}
